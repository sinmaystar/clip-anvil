...
- M6 Agent 阶段性闭环已落地：`/ws/agent`、Agent 对话 API、消息/事件/任务持久化、Eino checkpoint/resume、Producer 工具调用、HITL 决策卡、Storyboard/PSS、Craftsman/Worker 预览生成、Reviewer 重试调度、Composer 成片合成和 Agent 只读画布；MultiAgent 当前架构详见 [`engineering/agent-multiagent-architecture.md`](engineering/agent-multiagent-architecture.md)。
- 仍需后续完善：Studio/Agent 复制导入、Agent 长期记忆/Skill 配置化、生产级并发/成本控制、外部任务队列、Campaign/Strategist、完整音频生成链路、工具 schema 收紧和更多真实端到端回归。
- 数据库当前迁移到 `001_init_schema.sql` 到 `023_add_doubao_seedance_video_capability.sql`，真实 schema 以 `apps/server/migrations/` 和 sqlc 生成代码为准。
