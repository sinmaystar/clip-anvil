...
| 层 | 知道什么 | 不知道什么 |
|---|---|---|
| 用户界面层 | React Flow nodes/edges、画布坐标、视觉样式、事件渲染 | Agent 内部编排逻辑、生产翻译规则 |
...
- React Flow `MediaFlowNode`（M1 仅 text 节点，当前已扩展到多媒体节点）
- 画布右键菜单创建文本节点
- 节点拖拽位置持久化
...
- [画布设计和交互方案](canvas.md) — React Flow 投影、视觉规格、数据通路
