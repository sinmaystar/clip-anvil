...
| M11.1 HyperFrames 清理与 Motion 路由基础 | 移除活跃 HyperFrames/template route，新增 motion profile/capability。 | 迁移注册 `internal_motion_video/remotion-motion-shot-v1`；RenderPlan 支持 `motion_shot_video/image_to_motion_video`；活跃代码不再引用 `internal_template_video/hyperframes-html/template_video`。 | 相关 Go 单测通过；`rg` 只在 archive/superseded 文档或历史说明中命中 HyperFrames；`make server-test` 相关包通过。 |
